...
class main():
    ...
    def start(self):
        ...
        discovery_cell_types = [x.replace("_pvalue", "") for x in discovery_data_df if x.endswith("_pvalue")]
        ...
        for discovery_ct in discovery_cell_types:
            ...
            print("{}_pvalue".format(discovery_ct))
            print(discovery_beta_column)
            discovery_subset_df = discovery_df.loc[:, ["index", "{}_FDR".format(discovery_ct), discovery_beta_column, "DeconAllele"]].copy()
            discovery_subset_df.columns = ["index", "discovery FDR", "discovery beta", "discovery allele"]
            print(discovery_subset_df)

            print("Combine data")
            df = discovery_subset_df.merge(replication_df, on=["index"], how="right")
            print(df)
            ...
            print(df)
            print(df.loc[df["index"] == "ENSG00000188976_rs34903276", :])
            exit()
            ...
            self.plot_scatterplot(
                ...
                ylabel="{} (log beta)".format(self.replication_name),
                palette=self.colormap,
                filename="{}_signif_vs_{}_{}".format(self.discovery_name,
                                                     self.replication_name,
                                                     discovery_ct)
                )
            self.plot_scatterplot(
                ...
                ylabel="{} (log beta)".format(self.replication_name),
                palette=self.colormap,
                filename="{}_vs_{}_both_signif_{}".format(self.discovery_name,
                                                          self.replication_name,
                                                          discovery_ct)
            )
