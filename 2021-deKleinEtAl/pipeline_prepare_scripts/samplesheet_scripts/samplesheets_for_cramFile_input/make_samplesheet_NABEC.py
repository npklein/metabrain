...
parser = argparse.ArgumentParser(description='Make Molgenis Compute samplesheet for NABEC.')
parser.add_argument('samplesheet', help='NABEC samplesheet')
...
for study in study_individuals:
    ...
    for individual in individuals:
        ...
        if total_number_of_samples % batch_size == 0 or current_number_of_samples + len(samples_per_individual[individual]) > batch_size:
            ...
            out = open(args.samplesheet_dir+'/samplesheet_NABEC_RNA.'+study+'_'+batch+'.txt','w')
        ...
        for sample in sorted(samples_per_individual[individual]):
            ...
            if assay_and_library_type[sample][1] == 'PAIRED':
                R1 = args.fastq_dir+'/'+sample+'_1.fastq.gz'
                R2 = args.fastq_dir+'/'+sample+'_2.fastq.gz'
            elif assay_and_library_type[sample][1] == 'SINGLE':
                R1 = args.fastq_dir+'/'+sample+'.fastq.gz'

            out.write(sample+',NABEC,'+individual+','+R1+','+R2+','+cram+'\n')
