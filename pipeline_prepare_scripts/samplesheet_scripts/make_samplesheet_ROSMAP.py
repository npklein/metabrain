...
parser = argparse.ArgumentParser(description='Make Molgenis Compute samplesheet for ROSMAP.')
parser.add_argument('samplesheet', help='ROSMAP samplesheet from synapse')
...
with open(args.samplesheet) as input_file:
    ...
    for line in input_file:
        ...
        if index % 25 == 0:
            ...
            if out:
                out.close()
            out = open(args.samplesheet_dir+'samplesheet_ROSMAP_RNA.'+batch+'.txt','w')
            out.write('internalId,project,sampleName,reads1FqGz,reads2FqGz\n')
    
        out.write(sample_id+',ROSMAP,'+individual_id+','+fastq1+','+fastq2+'\n')
