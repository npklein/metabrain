...
fastq_location = {}
with open('/groups/umcg-biogen/tmp01/input/rawdata/AMP_AD/fastq/MayoCBE/SYNAPSE_METADATA_MANIFEST.tsv') as input_file:
    input_file.readline()
    for line in input_file:
        line = line.strip().split('\t')
        if '.r1.' in line[0]:
            fastq_location[line[2].split('_L')[0]] = line[0]
...
with open(args.samplesheet) as input_file:
    ...
    for line in input_file:
        ...
        if sample_id+'.'+line[8] not in fastq_location:
            continue
        fastq1 = fastq_location[sample_id+'.'+line[8]]
        ...
        out.write(sample_id+',MayoCBE,'+individual_id+','+fastq1+','+fastq2+'\n')
