...
adjust_workflows(){
    ...
    sed -i 's;DownloadFromENA,../protocols/DownloadFromENA.sh,;ConvertBamOrCramToFastq,../protocols/ConvertBamOrCramToFastq.sh,;' Public_RNA-seq_QC/workflows/workflow_brain_eQTL.csv
    sed -i 's;STARMapping,../protocols/STARMapping.sh,DownloadFromENA;STARMapping,../protocols/STARMapping.sh,ConvertBamOrCramToFastq;' Public_RNA-seq_QC/workflows/workflow_brain_eQTL.csv
    sed -i 's;FastQC,../protocols/Fastqc.sh,DownloadFromENA;FastQC,../protocols/Fastqc.sh,ConvertBamOrCramToFastq;' Public_RNA-seq_QC/workflows/workflow_brain_eQTL.csv

    # add last step to remove bams and original cram
    ...
    echo 'RemoveOriginalCramFile,../protocols/RemoveOriginalCramFile.sh,RemoveBamFiles;CreateCramFiles;FastQC;RemoveFastqFiles' >> Public_RNA-seq_QC/workflows/workflow_brain_eQTL.csv
...
parse_commandline(){
    ...
    if [ -z "$results_dir" ];
    then
        echo "ERROR: -z/--results_dir not set!"
        usagae
        exit 1;
    fi
