...
1_remove_duplicate_samples(){
    ...
    output_dir_step1="${output_dir}/1_removeDuplicates/$(basename ${expression_file%.txt.gz}).duplicateSamplesRemoved.txt.gz"
    if [ ! -f ${output_dir_step1} ];
    then
        echo "start step 1"
        bash $github_dir/GeneNetwork/scripts_per_step/1_remove_duplicate_samples.sh \
            -p $project_dir \
            -e $expression_file \
            -o $TMPDIR/$(basename $output_dir_step1) \
            -c $config_template_dir \
            -j $jar_dir
        echo "done"
        echo ""
        echo ""
        echo "Do: mv $TMPDIR/$(basename $output_dir_step1) $output_dir_step1"
        mkdir -p $(dirname $output_dir_step1)
        mv $TMPDIR/$(basename $output_dir_step1) $output_dir_step1
    fi
}

2_select_samples(){
    # Step 2. Select samples from expression file
    output_dir_step2="${output_dir}/2_selectSamples/$(basename ${expression_file%.txt.gz}).duplicateSamplesRemoved_extractedColumns.txt.gz"
    new_output_dir_step2=$output_dir/2_selectSamples/$(basename ${expression_file%.txt.gz}).duplicateSamplesRemoved_extractedColumnsnoVarianceRowsRemoved.txt.gz
    if [ ! -f ${new_output_dir_step2} ]
    then
        echo "start step 2"
        ...
        # change this
        f1=$TMPDIR/2_selectSamples/$(basename ${expression_file%.txt.gz}).duplicateSamplesRemoved_extractedColumns_noVarRemoved.txt.gz
        mv $f1 $TMPDIR/2_selectSamples/$(basename $new_output_dir_step2)
...
3_quantileNormalized(){
    ...
    then
        ...
        output_dir_step3=$output_dir/3_quantileNormalized/$(basename ${new_output_dir_step2%.txt.gz}.QuantileNormalized.txt.gz)
        if [ ! -f $output_dir_step3 ];
        then
            bash $github_dir/GeneNetwork/scripts_per_step/3_PCA_on_quantNormalizedData.sh \
                -p $project_dir \
                -e $new_output_dir_step2 \
...
4_DeseqNormalizedData(){
    output_dir_step4=$output_dir/4_deseqNormalized/$(basename ${new_output_dir_step2%.txt.gz}noVarianceRowsRemoved.DESeqNorm.txt.gz)
    if [ ! -f $output_dir_step4 ];
    then
...
5_RemoveCovariates(){
    output_dir_step5=$output_dir/5_covariatesRemoved/$(basename ${output_dir_step4%.txt.gz}.CovariatesRemoved.txt.gz)
    if [ ! -f $output_dir_step5 ];
    then
...
6_CorrelationMatrix(){
    output_dir_step6="$output_dir/6_correlation_matrix/MetaBrain.deseqNorm.covarCorrected.correlation.txt.gz"
    if [ ! -f $output_dir_step6 ];
    then
        # Step 6. Make correlation matrix
        bash $github_dir/GeneNetwork/scripts_per_step/6_CorrelationMatrix.sh \
            -p $project_dir \
            -e $output_dir_step5 \
            -o $TMPDIR/$(basename $output_dir_step6) \
            -c $config_template_dir \
            -j $jar_dir
        mkdir -p $(dirname $output_dir_step6)
        mv $TMPDIR/$(basename $output_dir_step6) $output_dir_step6
    fi
}
...
7_PCA_on_correlation(){
    ...
    then
        ...
        sed -i "s;REPLACEoutput_dir;$output_dir_step6;" $TMPDIR/7_PCA_on_correlation.sh
