...
GetOptions(
		...
		"writeOnlyPass:s"	=> \$writeOnlyPass #optional
);
...
while (my $line=<VCF>) {
    ...
    if ($line =~ m/^#.+/gs) { #Header line, just write to output
		...
		if((defined($writeOnlyPass) && $ok == 1) || !defined($writeOnlyPass)){
    	...
    	$lnctr = $lnctr + 1;
	if ($lnctr % 10000 == 0) {
		print "$inputFile: $lnctr lines parsed\n";
	}    
    }
}
...
--writeOnlyPass\t\t\t[OPTIONAL] Only write if variant passes filters
###################################################
