    ...
    def start(self):
        ...
        for row_index in range(n):
            ...
            for cov_index, cov_colname in enumerate(covs_df.columns):
                ...
                df_alt, rss_alt, coefficients_alt, std_errors_alt = self.create_model(alt_matrix, expression, cols=[inter_name])
        ...
        for covariate, result in results.items():
            ...
            df = pd.DataFrame(result, columns=["eQTL", "coef", "std", "f-value", "p-value"])
            df = df.set_index('eQTL')
            print(df)
            df.to_csv(os.path.join(self.outdir, "test_interaction_eqtl_{}.txt.gz".format(covariate)), compression="gzip", sep="\t", index=True, header=True)
