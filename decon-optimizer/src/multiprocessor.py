...
class MultiProcessor:
    ...
    def multiprocessing(self, work_func, samples=None):
        if samples is None:
        ...
        while n < len(samples):
            ...
            if n != previous_n:
                ...
                self.log.info("\t\t\t[{}:{}:{}] {}/{} samples completed [{:.2f}%]".format(int(rt_hour), int(rt_min), int(rt_sec), n, len(samples), (100 / len(samples)) * n))
                previous_n = n
